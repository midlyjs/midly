...
class Midly extends ((options.key && options.cert || options.pfx)? https : http).Server {
	constructor() {
		super(options, async (req, res) => {
			...
			try {
				...
				this.emit('error', error, req, res, ctx)
			}
		...
		this.once('newListener', event => {
			if(event == 'error') this.removeListener('error', this.defaultErrorHandler)
		})
	}
